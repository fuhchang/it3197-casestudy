 ...
 *         http://www.linz.govt.nz/geodetic/conversion
 *         -coordinates/projection-conversions
...
public class SVY21 {
	...
	private static double calcM(double lat) { // M: meridian distance.
		...
		double m = a
				* ((A0 * latR) - (A2 * Math.sin(2 * latR))
						+ (A4 * Math.sin(4 * latR)) - (A6 * Math.sin(6 * latR)));
		return m;
	}

	private static double calcRho(double sin2Lat) { // Rho: radius of curvature
													// of meridian.
		double num = a * (1 - e2);
	...
	private static double calcV(double sin2Lat) { // v: radius of curvature in
													// the prime vertical.
	 ...
	 *
	 * This method returns an immutable LatLonCoordiante object that contains
	 * two fields, latitude, accessible with .getLatitude(), and longitude,
	 * accessible with .getLongitude().
	 *
	 * @param N
	...
	public static LatLonCoordinate computeLatLon(double N, double E) {
		...
		// powers.
		double latPrimeT1 = ((3 * n / 2) - (27 * n3 / 32))
				* Math.sin(2 * sigma);
		double latPrimeT2 = ((21 * n2 / 16) - (55 * n4 / 32))
				* Math.sin(4 * sigma);
		double latPrimeT3 = (151 * n3 / 96) * Math.sin(6 * sigma);
		double latPrimeT4 = (1097 * n4 / 512) * Math.sin(8 * sigma);
		double latPrime = sigma + latPrimeT1 + latPrimeT2 + latPrimeT3
				+ latPrimeT4;
		...
		double latTerm2 = latFactor
				* ((Eprime * x3) / 24)
				* ((-4 * psiPrime2 + (9 * psiPrime) * (1 - tPrime2) + (12 * tPrime2)));
		double latTerm3 = latFactor
				* ((Eprime * x5) / 720)
				* ((8 * psiPrime4) * (11 - 24 * tPrime2) - (12 * psiPrime3)
						* (21 - 71 * tPrime2) + (15 * psiPrime2)
						* (15 - 98 * tPrime2 + 15 * tPrime4) + (180 * psiPrime)
	 ...
	 * accessible with .getLongitude().
	 *
	 * This method is a shorthand for the functionally identical public
	 * LatLonCoordinate computeLatLon(double N, double E).
	 *
	 * @param coord
	 *            an SVY21Coordinate object to convert.
	 * @return the conversion result a LatLonCoordinate.
	 */
	 ...
	 * fields, northing, accessible with .getNorthing(), and easting, accessible
	 * with .getEasting().
	 *
	 * @param lat
	 *            latitude in degrees.
	...
	public static SVY21Coordinate computeSVY21(double lat, double lon) {
		// Naming convention: sin2Lat = "square of sin(lat)" =
		// Math.pow(sin(lat), 2.0)
		double latR = lat * radRatio;
		...
		double nTerm4 = w8 / 40320 * v * sinLat * cos7Lat
		...
		double eTerm2 = w4
				/ 120
				* cos4Lat
				* ((4 * psi3) * (1 - 6 * t2) + psi2 * (1 + 8 * t2) - psi * 2
						* t2 + t4);
	 ...
	 *
	 * This method is a shorthand for the functionally identical public
	 * SVY21Coordinate computeSVY21(double lat, double lon).
	 *
	 * @param coord
	 *            a LatLonCoordinate object to convert.
